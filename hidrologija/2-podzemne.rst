...
Приметимо да је на слободној површини :math:`\phi=y`. Пошто се облик слободне површине не зна, то је њено одређивање посебан задатак у овој области. И овај проблем ћемо покушати да покријемо ФЗНН методом. 


Стационарно струјање кроз порозну средину
---------------------------------------------------------------------

Дводимензионални стационарни ток кроз порозни медијум је регулисан константном разликом потенцијала на две површине. Проток се јавља између два непропусна слоја у правоугаоној геометрији димензија *2m x 2m*. Пoставка проблема је прилично једноставна и њени најважнији делови се налазе на следећем листингу. 
...
који ћемо добити тиме што додамо :math:`\frac{\pi}{2}` правцу тангенте на `phi` коју израчунавамо захваљујући тривијалној доступности првог извода у ФЗНН методологији. Након тога лако израчунавамо компоненте нормале `nx` и `ny`. Гранични услов слободне површине постављамо на исти начин као и раније када смо користили библиотеку SCIANN, тј. у виду коњункције се наводи где услов важи и шта у том делу домена важи. Међутим, овог пута немамо строго дефинисане координате, јер положај слободне површине и не знамо. Оно што знамо је да је на читавој слободној површини :math:`\Phi=y`, па ово наводимо као област важења:
...
и :math:`L=x_2-x_1`. Дакле, као и код проблема без слободне површине, потенцијално поље је униформно у односу на *Y* координату, тј. не зависи од ње под условом да се налазимо испод слободне површине. Поређење ФЗНН решења са аналитичким решењем може се видети на 
...
.. figure:: 2d-filtracija-sp1.png
    ...
    ФЗНН решење потенцијала дуж *X* осе за 2Д случај филтрације са слободном површином.
...
.. figure:: 2d-filtracija-sp2.png
    ...
    ФЗНН решење поља потенцијала за 2Д случај филтрације са слободном површином.

Може се приметити релативно добро слагање ФЗНН решења са аналитичким решењем, као и очигкледна разлика распореда поља потенцијала у односу на случај без слободне површине приказан на :numref:`bez-slobodne-povrsine-res2`. Ако пак упоредимо приступ решавању проблема слободне површине ФЗНН методом и класичном методом коначних елемената као код :cite:t:`kojic1998metod`, можемо приметити да је ФЗНН приступ значајно лакши. Разлог томе је што се код ФЗНН не захтева никакав посебан нумерички третман и употреба нумеричких претпоставки, већ се физика проблема директним путем преводи у ФЗНН гранични услов. 
