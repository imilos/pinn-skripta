...
.. figure:: 2d-filtracija-bez-sp1.png
    ...
    ФПНМ решење потенцијала дуж *X* осе за 2Д случај филтрације без слободне површине.

Поређење аналитичког и ФПНМ решења је приказано на :numref:`bez-slobodne-povrsine-res1`, а потенцијално поље је приказано на :numref:`bez-slobodne-povrsine-res2`. Униформност потенцијалног поља у *Y* смеру, показује тачност 2Д ФПНМ решења за овај стационарни проблем.
...
који ћемо добити тиме што додамо :math:`\frac{\pi}{2}` правцу тангенте на ``phi`` коју израчунавамо захваљујући тривијалној доступности првог извода у ФПНМ методологији. Након тога лако израчунавамо компоненте нормале ``nx`` и ``ny``. Гранични услов слободне површине постављамо на исти начин као и раније када смо користили библиотеку SCIANN, тј. у виду коњункције се наводи где услов важи и шта у том делу домена важи. Међутим, овог пута немамо строго дефинисане координате, јер положај слободне површине и не знамо. Оно што знамо је да је на читавој слободној површини :math:`\Phi=y`, па ово наводимо као област важења:
...
и :math:`L=x_2-x_1`. Дакле, као и код проблема без слободне површине, потенцијално поље је униформно у односу на *Y* координату, тј. не зависи од ње под условом да се налазимо испод слободне површине. Поређење ФПНМ решења са аналитичким решењем може се видети на 
...
.. figure:: 2d-filtracija-sp1.png
    ...
    ФПНМ решење потенцијала дуж *X* осе за 2Д случај филтрације са слободном површином.
...
Може се приметити релативно добро слагање ФПНМ решења са аналитичким решењем, као и очигкледна разлика распореда поља потенцијала у односу на случај без слободне површине приказан на :numref:`bez-slobodne-povrsine-res2`. Ако пак упоредимо приступ решавању проблема слободне површине ФПНМ методом и класичном методом коначних елемената као код :cite:t:`kojic1998metod`, можемо приметити да је ФПНМ приступ значајно лакши. Разлог томе је што се код ФПНМ не захтева никакав посебан нумерички третман и употреба нумеричких претпоставки, већ се физика проблема директним путем преводи у ФПНМ гранични услов. 
