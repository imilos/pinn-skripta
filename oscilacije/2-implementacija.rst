...
.. code-block:: python
    ...
    # Definisanje problema, granicnih uslova, broja kolokacionih tacaka
...
представља поставку проблема у свом изворном облику једначине :math:numref:`eq:oscilacije-dif`. Овде је очигледна једна од главних предности НМПФЗ, тј. да неке додатне трансформације у интеграционим тачкама нису потребне, већ само поставка диференцијалне једначине у виду функције губитка и граничних услова у истом облику. Услужне методе ``dde.grad.jacobian`` и ``dde.grad.hessian`` враћају прве, односно друге изводе по улазним варијаблама примењујући тзв. аутоматску диференцијацију. Подразумевано се у позадини користи *Tensorflow* за тензорске операције ниског нивоа. 
